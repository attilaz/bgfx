// Autogenerated from filament shader https://github.com/google/filament
/*
...
void toTangentFrame(const highp vec4 q, out highp vec3 n, out highp vec3 t) {
 ...
 static vec4 mesh_color;
#endif

#if defined(HAS_ATTRIBUTE_UV0)
 static vec2 mesh_uv0;
#endif

#if defined(HAS_ATTRIBUTE_UV1)
 static vec2 mesh_uv1;
#endif
 ...
 static vec4 mesh_custom0;
#endif

#if defined(HAS_ATTRIBUTE_CUSTOM1)
 static vec4 mesh_custom1;
#endif

#if defined(HAS_ATTRIBUTE_CUSTOM2)
 static vec4 mesh_custom2;
#endif

#if defined(HAS_ATTRIBUTE_CUSTOM3)
 static vec4 mesh_custom3;
#endif

#if defined(HAS_ATTRIBUTE_CUSTOM4)
 static vec4 mesh_custom4;
#endif

#if defined(HAS_ATTRIBUTE_CUSTOM5)
 static vec4 mesh_custom5;
#endif

#if defined(HAS_ATTRIBUTE_CUSTOM6)
 static vec4 mesh_custom6;
#endif

#if defined(HAS_ATTRIBUTE_CUSTOM7)
 static vec4 mesh_custom7;
#endif

 static highp vec3 vertex_worldPosition;
#if defined(HAS_ATTRIBUTE_TANGENTS)
 SHADING_INTERPOLATION static mediump vec3 vertex_worldNormal;
#if defined(MATERIAL_HAS_ANISOTROPY) || defined(MATERIAL_HAS_NORMAL) || defined(MATERIAL_HAS_CLEAR_COAT_NORMAL)
 SHADING_INTERPOLATION static mediump vec3 vertex_worldTangent;
 SHADING_INTERPOLATION static mediump vec3 vertex_worldBitangent;
#endif
#endif

#if defined(HAS_ATTRIBUTE_COLOR)
 static mediump vec4 vertex_color;
#endif

#if defined(HAS_ATTRIBUTE_UV0) && !defined(HAS_ATTRIBUTE_UV1)
 static highp vec2 vertex_uv01;
#elif defined(HAS_ATTRIBUTE_UV1)
 static highp vec4 vertex_uv01;
#endif
...
float evaluateSSAO() {
    ...
    vec2 uv = s_FragCoord.xy * frameUniforms.resolution.zw;
...
void filamentSetWorldPosition(vec3 _worldPosition)
{
    shading_position = _worldPosition;
    shading_view = normalize(frameUniforms.cameraPosition - _worldPosition);
}


void filementSetNormal(vec3 _worldNormal, bool _frontFacing)
{
#if defined(MATERIAL_HAS_DOUBLE_SIDED_CAPABILITY)
    if (isDoubleSided()) {
        _worldNormal = _frontFacing ? _worldNormal : -_worldNormal;
    }
#endif

    // Leave the tangent and bitangent uninitialized, we won't use them
    shading_tangentToWorld[2] = normalize(_worldNormal);
}

void filementSetTangentSpace(vec3 _worldNormal, vec3 _worldTangent, vec3 _worldBitangent, bool _frontFacing)
{
#if defined(MATERIAL_HAS_DOUBLE_SIDED_CAPABILITY)
    if (isDoubleSided()) {
        _worldNormal = _frontFacing ? _worldNormal : -_worldNormal;
    ...
    shading_tangentToWorld = mat3(
            normalize(_worldTangent), normalize(_worldBitangent), normalize(_worldNormal));
}

/*
 * Returns a pre-exposed HDR RGBA color in linear space.
 */
vec4 filamentEvaluate(const MaterialInputs _inputs)
{
